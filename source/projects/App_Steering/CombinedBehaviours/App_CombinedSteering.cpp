...
#include "projects\App_Steering\CombinedBehaviours\Obstacle.h"

App_CombinedSteering::~App_CombinedSteering()
{
	SAFE_DELETE(m_pBlendedSteerinf);
...
void App_CombinedSteering::Start()
{
	...
	m_pBlendedSteerinf = new BlendedSteering({ { m_pSeek , 0.5f }, { m_pDrunkWander , 0.5f } });


	m_pDrunkAgent = new SteeringAgent();
	m_pDrunkAgent->SetSteeringBehavior(m_pBlendedSteerinf);
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	//UI
	{
		...
		ImGui::Spacing();
		ImGui::SliderFloat("Seek", &m_pBlendedSteerinf->m_WeightedBehaviors[0].weight, 0.f, 1.f, "%.2");
		ImGui::SliderFloat("Wander", &m_pBlendedSteerinf->m_WeightedBehaviors[1].weight, 0.f, 1.f, "%.2");
