...
std::vector<Elite::Vector2> App_NavMeshGraph::FindPath(Elite::Vector2 startPos, Elite::Vector2 endPos)
{
	...
	const Triangle* beginTriangle = m_pNavGraph->GetNavMeshPolygon()->GetTriangleFromPosition(startPos);
	...
	auto cloneGraph = m_pNavGraph->Clone();
		
	//Create extra node for the Start Node (Agent's position)
	NavGraphNode* startNode = new NavGraphNode(cloneGraph->GetNextFreeNodeIndex(), -1, startPos);
	cloneGraph->AddNode(startNode);

	for (auto edge : beginTriangle->metaData.IndexLines)
	{
		int nodeIdx{m_pNavGraph->GetNodeIdxFromLineIdx(edge)};
		if (nodeIdx != invalid_node_index)
		{
			Elite::GraphConnection2D* con = new Elite::GraphConnection2D(startNode->GetIndex(), nodeIdx);
			con->SetCost(Elite::Distance(cloneGraph->GetNode(nodeIdx)->GetPosition(), cloneGraph->GetNodePos(startNode)));
			cloneGraph->AddConnection(con);
		}
	}
	...
	for (auto edge : endTriangle->metaData.IndexLines)
	{
		...
		if (nodeIdx != invalid_node_index)
		{
			Elite::GraphConnection2D* con = new Elite::GraphConnection2D(endNode->GetIndex(), nodeIdx);
			con->SetCost(Elite::Distance(cloneGraph->GetNode(nodeIdx)->GetPosition(), cloneGraph->GetNodePos(endNode)));
			cloneGraph->AddConnection(con);
		}
	}
	...
	auto astarPathfinder = FlowField<NavGraphNode, GraphConnection2D>(cloneGraph.get(), m_pHeuristicFunction);

	for (auto pos : astarPathfinder.FindPath(startNode, endNode))
	{
		m_DebugNodePositions.push_back(pos->GetPosition());
		finalPath.push_back(pos->GetPosition());
	}

	//Extra: Run optimiser on new graph, Make sure the A star path is fine before uncommenting this!
	m_Portals = SSFA::FindPortals(astarPathfinder.FindPath(startNode, endNode) , m_pNavGraph->GetNavMeshPolygon());
