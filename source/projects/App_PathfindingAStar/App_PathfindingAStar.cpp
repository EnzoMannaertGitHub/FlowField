...
void App_PathfindingAStar::Update(float deltaTime)
{
		...
		&& startPathIdx != endPathIdx)
	{
		...
		auto astarPathfinder = FlowField<GridTerrainNode, GraphConnection>(m_pGridGraph, m_pHeuristicFunction);
		m_vPath = astarPathfinder.FindPath(startNode, endNode);
...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	{
		m_GraphRenderer.RenderHighlightedGrid(m_pGridGraph, { m_pGridGraph->GetNode(endPathIdx) }, END_NODE_COLOR_BLUE);
	}
