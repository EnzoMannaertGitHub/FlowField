...
namespace Elite
{
	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			{
				//Local current portal
				const auto &portal = portals[i];

				//--- RIGHT CHECK ---
				//1. See if moving funnel inwards - RIGHT
				auto newRightLeg = portals[i].Line.p2 - apex;

					if (Elite::Cross(newRightLeg, rightLeg) >= 0)
					{
						if (Elite::Cross(leftLeg , newRightLeg) < 0)
						{
							//if we cross over left leg
							apex += leftLeg;
							apexIndex = leftLegIndex;

							//create the new iterator
							int newIterator{ apexIndex + 1 };
							i = newIterator;
							...
							{
								rightLeg = portals[rightLegIndex].Line.p2 - apex;
								leftLeg = portals[leftLegIndex].Line.p1 - apex;
								continue;
							}
						}
						else
						...
						//if we don't cross over left leg
				...
				auto newLeftLeg = portals[i].Line.p1 - apex;
